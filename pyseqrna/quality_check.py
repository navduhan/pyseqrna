...
def fastqcRun(sampleDict=None, configFile=None,slurm=False, mem=10, cpu=8, task=1, paired =False, out ='fastqc_results', outDir="pySeqRNA_results", dep=''):
    ...
    if configFile != None:
        ...
        pl.info("Using default config file fastqc.ini")
    ...
    for key, value in sampleDict.items():  # Iterate thorough total number of samples
        ...
        if paired:
            ...
            try:
                ...
                pl.error("please provide a valid input data path")
            
            inputPair = pu.get_basename(value[2])

        try:

            execPATH = os.popen('which fastqc').read(
            ).rstrip()  # get absolute path of flexbar

        except Exception:

            pl.error("fastqc command not found in path")
            sys.exit()
        ...
        if slurm == True:  # check if slurm job scheduling is enabled or not
            ...
            try:
                job = pu.clusterRun(
                    'fastqc', fastqcCmd, mem=mem, cpu=cpu, tasks=task, dep=dep)
                ...
                pl.error("Slurm job sumission failed")
            ...
            try:
                ...
                pl.info("Job submition failed for {} error present in fastqc.err  ".format(
                    pu.get_basename(out)))
            finally:
