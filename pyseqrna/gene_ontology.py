...
class GeneOntology:
    ...
    def __init__(self, species=None, type=None):
    ...
    def enrichGO( self, file= None, pvalueCutoff=0.05, plot=True, plotType= 'dotplot', nrows=20,colorBy='logPvalues'):
        ...
        df_List = self.df[['ID', 'Term', 'Ontology', 'Function']].values.tolist()
        KOdescription = {}
        ...
        if end.shape[0]>1:
            ...
            if plot:
                if plotType == 'dotplot':
                    fig = self.dotplotGO(end,nrows,colorBy)
                if plotType == 'barplot':
                    fig = self.barplotGO(end,nrows,colorBy)
